...
AddEventHandler("auction:claimArea", function(id, title, initialValue, data)
    ...
    SetTimeout(config.AuctionDuration * 1000, function()
        ...
        local oldOwner = QBCore.Functions.GetPlayerByCitizenId(area.data.player)
        local newOwner = QBcore.Funcitons.GetPlayerByCitizenId(area.data.bid.player)

        if newOwner.money['bank'] < area.data.bid.value then
        ...
        local license = QBCore.Functions.GetIdentifier(src)
        ...
        if area.data.data.plate ~= "ADMIN" then
            ...
            MySQL.update("UPDATE player_vehicles SET citizenid = ?, license = ?, WHERE plate = ?", {newOwner.citizenid, license, area.data.data.plate})
        else
